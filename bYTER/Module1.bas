...
Public Function Open_File1(hwnd As Long) As String
   '
   Dim OpenFileDialog As OPENFILENAME
   Dim rv As Long
   ...
   If (rv&) Then
      Open_File1 = Trim$(OpenFileDialog.lpstrFile)
   Else
      Open_File1 = ""
   End If
...
Close #1
Close #2
For lv = 1 To ofsize Step 4 'Without Step its hella slow (you can increase the steps but dont forget to increas the code below too (you can add another loop too..)
        If of(lv) <> pf(lv) Then
        ...
        End If
            If of(lv + 3) <> pf(lv + 3) Then
        OffsetBox.AddItem hex(lv + 3)
        OriginalByte.AddItem hex(of(lv + 3))
        ChangedByte.AddItem hex(pf(lv + 3))
        End If
